...
由於 Bazel/Kleaf 採用隔離的沙盒進行編譯，**不能**直接使用 `setup.sh` 預設產生的跨目錄軟連結。必須在執行 `setup.sh` 後，將 KernelSU 的原始碼與 UAPI 檔案以**實體目錄**形式複製到 `common/` 中：
...
2. **執行 setup.sh 初始化設定**：
   ...
   ```bash
   KernelSU/kernel/setup.sh v3.2.0
   ```
...
3. **將軟連結替換為實體目錄**：
   ...
   # 2. 將 KernelSU/kernel 目錄實體複製到 common/drivers/kernelsu
...
* 修改 `common/drivers/kernelsu/Kbuild` (新版 KernelSU) 或 `common/drivers/kernelsu/Makefile` (舊版 KernelSU)：

   ```makefile
   # 將原來的 ccflags-y += -DKSU_VERSION=16 替換為計算出的真實版本號：
   ccflags-y += -DKSU_VERSION=32179  # 改成你計算出的數字
   ```
...
這一章只針對 **KernelSU v3.2.0（含）之後**。從這個版本開始，KernelSU 的 x86_64 syscall hook 會檢查 kernel 是否提供 `X86_FEATURE_INDIRECT_SAFE`；如果使用 KernelSU v3.2.0 以前的版本，通常不會遇到這個檢查。

處理方式要看 GKI kernel 的 syscall 實作，不只看 Android 大版本：

| AVD / GKI | syscall 實作 | KernelSU v3.2.0+ 作法 |
| --- | --- | --- |
...
適用 `common-android15-6.6`、`common-android16-6.12`，以及之後同樣採用 switch-case syscall hardening 的 x86_64 GKI。這條路徑要**保留 KernelSU 的 `X86_FEATURE_INDIRECT_SAFE` 檢查**，並依 kernel 版本套用對應 patch，讓 kernel 支援 `X86_FEATURE_INDIRECT_SAFE` 與 boot argument `syscall_hardening=off`。
...
這組 patch 會加入 `X86_FEATURE_INDIRECT_SAFE`，並提供 boot argument `syscall_hardening=off`。之後啟動 A15+ AVD 時需要加上這個 boot argument。
...
適用 `common-android13-5.15` 和 `common-android14-6.1`。這類 kernel 沒有新版 switch-case syscall hardening，不需要套 `X86_FEATURE_INDIRECT_SAFE` kernel patch；如果使用 KernelSU v3.2.0+，請在整合 KernelSU 之後把 KernelSU 端的檢查註解或移除。
...
### 2. 修正 x86_64 include 相容性 (還不確定哪些版本需要，有遇到下面的編譯錯誤的話再使用。)
